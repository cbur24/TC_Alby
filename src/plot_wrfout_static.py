...
# The following script takes as an input the output file (wrfout_d01*) from a wrf real run, and produces plots that demonstrate key indicators of its performance in simulating a cyclone (for a single given time-slice).
...
# The user needs to go to the 'user inputs' section and enter all the information required. To help with creating the vertical cross section through the eye of the TC, follow the instructions below to create a file containing the path of the modelled cyclone. This is used to automate the location of the cross-section in the subsequent script:
...
ncfile_path = input(" Input the filepath to the data (eg. data/is/here/wrfout_d0*) :  ")
ncfilepath = str(ncfile_path)

domainID = input(" What is the domain identifier? eg. d02 : ")
domainID = str(domainID)

time = input(" which time slice do you want (FORMAT MUST BE: 1978-04-05T00:00) :  ")
time = str(time)

gph_pressure = input("what pressure (in hPa) do you wish to see the geopoential height, windspeed and vorticity? eg. 500 : ")
gph_pressure = int(gph_pressure)

TC_modelled_path_loc = input("enter the filepath of the .csv that contains the vortex tracking TC path \n eg. csv/is/here/ATCF_track.csv : ")
TC_modelled_path_loc = str(TC_modelled_path_loc)

width = input("enter the width of the desired cross-section in grid units \n(better if this is an even number, 20 is about right) :  ")
width = int(width)

maxWindSpeeds = input("Do you want a facet plot of maximum wind speeds? Enter True or False : ")
if maxWindSpeeds == "True":
        maxWindSpeeds = True
if maxWindSpeeds == "False":
        maxWindSpeeds = False

if maxWindSpeeds == True:
	time_1 = input("Enter a time for the facet plot,there are going to be FOUR times to enter(FORMAT MUST BE: 1978-04-05T00:00) :  ")
	time_2 = input("enter the second time for the facet plot :  ")
	time_3 = input("enter the third time for the facet plot :  ")
	time_4 = input("enter the fourth time for the facet plot :  ")
	
	time_ws = [time_1, time_2, time_3, time_4]

if maxWindSpeeds == True:
	ts_plots = input("Do you want max wind speed timeseries plots for individual towns/cities? True or False : ")
	if ts_plots== "True":
        	ts_plots= True
	if ts_plots== "False":
        	ts_plots= False
	if ts_plots == True:	
		cities = [str(x) for x in input("Enter the names of FOUR cities as a comma seperated list eg. perth,geraldton :  ").split(",")]
		city_lats = [float(x) for x in input("Enter the latitudes of the four cities as a comma seperated list eg. -23.1,-36.0 : ").split(",")]
		city_longs = [float(x) for x in input("Enter the longitudes of the four cities as a comma seperated list eg. -100.1,-105.2 : ").split(",")]
	
		
	add_station_data = input("If you have station data that you want plotted on the timeseries plots for comparison, \n then set 'add_station_data' to True, but be careful, this only works because I curated \n the station data for my specific purposes so ordinarly you should set this to false. enter True or False : ")
	if add_station_data== "True":
		add_station_data= True
	if add_station_data== "False":
		add_station_data= False	
	if add_station_data== True:
		stationID  = [int(x) for x in input("Enter the 4 station IDs as a comma seperated lis eg 8051,9021 : ").split(",")]
		
results_path = input("where should I export the results eg. export/results/here/ : ")
